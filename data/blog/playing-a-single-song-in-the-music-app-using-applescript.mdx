...
excerpt: How to play a single song in the macOS Music app using AppleScript.
...
_Photo by_ [Nikolai Chernichenko](https://unsplash.com/@perfectcoding) _on_
...
the path to zsh executables. `/bin` stands for `binary` — a directory of all
...
because tje script is specifically targeting the artist `Spirit Nation` AND
...
`"Artists" -> "Creed"`. First there is the album "Human Clay", then "My Own
