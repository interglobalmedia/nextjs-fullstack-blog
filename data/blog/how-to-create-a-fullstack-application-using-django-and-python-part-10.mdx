...
lastModified: '2024-09-09T02:46:43.185Z'
...
`User` object is `passed` as an `argument` to `ManyToManyField()`, `because` the
`user` is the one to `"like"` the `topic post`. We will `go over` this in
`detail later`. This `section` is about `updating` the `UML diagrams` and
`wireframe`(`s`) to `include` the `PostLike` model.
