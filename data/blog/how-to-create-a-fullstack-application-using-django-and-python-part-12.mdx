...
lastModified: '2026-07-16T11:57:09.958Z'
...
image: pexels-realtoughcandy-11035474.webp
tags:
    [
...
](pexels-realtoughcandy-11035474.webp)

_Photo by [RealToughCandy.com](https://www.pexels.com/@realtoughcandy/) on
[pexels.com](https://www.pexels.com/photo/person-holding-an-orange-and-blue-python-sticker-11035474/)_
...
In Part 11, I created tests for `BoardTopics` using `class BoardTopicsTests`.
Now I am going to create tests for the Index (Home) page. Update
`class IndexTests(TestCase)` to the following in `boards/tests.py`:
...
I have added a `setUp(self)` method for IndexTests. That's because I am also
going to need a Board instance for IndexTests. I moved `url` and `self.response`
into `setUp(self)` so I can reuse the same response in the new
`test_home_view_contains_link_to_topics_page` test.

In `test_home_view_contains_link_to_topics_page`, I use the `.assertContains()`
method. Django adds `.assertContains()` to its test suite. It is used
specifically for responses. `.assertContains()` asserts that the response
(`index.html`) contains `'href="{0}"'.format(board_topics_url)`, which
transforms to `'href="/boards/1/"'`. However, if I run `python3 manage.py test`
as the code is right now, I get back the following:
...
In order to get my failed test to pass, I have to update `templates/index.html`:
...
I changed `{{ board.name }}` to
`<a href="{% url 'board_topics' board.id %}" >{{ board.name }}</a >`.

Now when I run the tests again using the `python3 manage.py test` command, the
...
I should always use the Django `{% url %}` template tag to create application
URLs. The first parameter is the name of the URL defined in `URLconf/urls.py`,
and the second argument is the `board.id`.

### Viewing the updated HTML templates in the browser

_Next_, I can check what my changes actually look like in the browser:

![List of linked available Boards on the index/home page](Screenshot-2024-09-11-at-12.47.07-PM.webp)
...
![Linked single Board page](Screenshot-2024-09-11-at-12.51.42-PM.webp)

_Linked single Board page_

I am taken to `http://127.0.0.1:8000/boards/1/`, and that URL is why my failed
test no longer fails. The `path/url` to the single Board now contains
`/boards/1/`.
...
Let's link back to the `index/home` page from the single Board page. First I
will write the test for that in `boards/tests.py`:
...
I can run my tests again using `python3 manage.py test`, and something like the
following should be returned:
...
I couldn't find `/` because I have to update my `topics.html` template:
...
The test passes because I have added `<a href="{% url 'index' %}">Boards</a>`.
`'index'` translates to `/`. `index` is the `name` of the `path` of the `index`
or home page named in `django_boards/urls.py`.

Now the single Board page links back to the `index/home` page:

![Single Board page links back to index/home page](Screenshot-2024-09-1-at-1.57.35-PM.webp)
...
[01ba9fc](https://github.com/interglobalmedia/django-boards/commit/01ba9fc3c53ad323f7f6c6d01c6f6aabe36a2a8d).

## Conclusion

In this section, I add the IndexTests class to boards/tests.py, update
templates/index.html to make a failed test pass, write a test for linking back
to the index/home page from the single Board page, update templates/topics.html
to link back to the index/home page, and update templates/topics.html so a
failed test would pass.
