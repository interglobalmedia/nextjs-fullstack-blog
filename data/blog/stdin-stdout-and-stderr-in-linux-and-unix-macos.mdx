...
stdin, stdout, and stderr are three standard data streams that one creates when
executing a command in Linux. And what is a stream? According to opensource.com,
...
stdin is represented by File handle 0, stdout is represented by File handle 1,
and stderr is represented by File handle 2.
...
`files`. We can `read` a `file` or `data` into a `file`.
...
`ls macos-scripting linux-scripting` would all be redirected to `/dev/null` and
discarded.
