...
-   [Vim modes](#vim-modes)
    -   [Normal mode](#normal-mode)
        ...
        -   [The w shortcut](#the-lowercase-w-shortcut)
...
the `x` key, and the `character` is `deleted`. This `shortcut` is also
...
[the yy shortcut](#the-yy-shortcut), I `press` the `p` key, and the `line` is
`pasted` below the `line` I `copied`.
