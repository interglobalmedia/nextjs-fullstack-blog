...
lastModified: '2026-05-29T14:08:56.106Z'
title:
    'Making and verifying a cleartext digital signature using a PGP key pair in
    Linux'
author: 'Maria D. Campbell'
excerpt:
    'How to make and verify a cleartext digital signature using a PGP key pair
    in Linux.'
...
In order to create a cleartext signature, I run the following command in
Terminal:
...
I still am prompted to provide the passphrase/password for the PGP key pair I am
using.
...
The text content itself is not encrypted in any way and `gpg` is not needed to
...
To verify a cleartext signature, I (or a real world recipient of the file) runs
...
`gpg --verify clear_text.txt.sig` again, the following is returned in Terminal:
