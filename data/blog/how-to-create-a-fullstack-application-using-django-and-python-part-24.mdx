...
lastModified: '2026-05-25T22:23:47.084Z'
...
Now we are going to add Markdown to the `reply_topic` and `edit_post` views and
templates.
...
We are dealing with user input here, so we have to be careful. When using the
markdown function, we are telling it to escape special characters first and THEN
parse the markdown. After that, we mark the output string as safe to be used in
the template.
...
JavaScript I had already added to the project. And we are also using two third
party JavaScript libraries from Bootstrap that could cause conflicts. There is
much to refactor, but in the end, the code will be slimmer and easy to modify if
needed or desired.
...
Right now the associated html markup in `templates/topic_posts.html` is the
following:
...
The only thing we don't need is the `div` element with the `id` of `"feedback"`,
...
**Note 10.10.24**: I inadvertently removed the Popper script tag from the script
tags in` templates/base.html`, but will be adding it back and pushing the change
to GitHub momentarily. All the script tags are included at the end of this post
when I add the `app.js` file. Sorry for this inconvenience!
...
Again, we don't need the `div` element with the `id` of `"feedback"`, so the
...
in their own JavaScript file. Right now the code for both resides in a file
called `scroll-top-bottom.js`, and it contains the following:
...
it's part. Right now it looks like the following:
...
Now the structure and contents of the `static/js` directory looks like the
following:
...
In order to make ES6 modules actually work in our Django Boards website, we have
...
modules to our JavaScript code. This meant refactoring the copy (link) button
and associated html markup and CSS. I refactored and modularized the scroll top
and bottom button JavaScript code and associated CSS, modularized the pagination
