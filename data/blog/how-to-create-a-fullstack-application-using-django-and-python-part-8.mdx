...
lastModified: '2024-09-06T17:13:22.089Z'
title: 'How to create a fullstack application using Django and Python Part 8'
author: 'Maria D. Campbell'
excerpt: 'How to create a fullstack application using Django and Python part 8.'
image: board-app.jpg
...
This next thing to do is to make rendering the boards cleaner and more dynamic:
...
And this would result in the following in the browser:

![Rendering board names in the browser](Screenshot-2024-09-06-at-2.11.07-PM.jpg)

We changed the code from using the render method to render the index.html page
with the list of boards (very simple and limited way of doing this) to returning
an HttpResponse and passing the response_html object to it. This is okay, but it
is the role of the templating engine to render the page(s).
...
iterates over the keys of an object. `{{ board.name }}` renders the board name
to the page, for example.

Before we can actually view our index.html page being rendered to the page, we
have to tell Django where to find this template.

To do this, we have to go into settings.py located in the django_boards project
...
What the value of the 'DIRS' array is doing is finding the full path of our
...
And then we type the following in the python3 shell:
...
Next, we are going to write our first test in the tests.py page inside the
boards app:
...
If I had a mistake in my code like an uncaught exception, syntax error, etc.,
Django would return a status code of 500. A status code of 500 gives a message
of "Internal Server Error". A 500 status code means the server encountered
something it didn't expect and was unable to complete the request.
...
project and app. However, eventually I should have removed the `__init__.py`
...
we may want to use for the frontend of the site.
...
django.contrib.staticfiles collects static files from each of our applications
(and any other places we specify) into a single location that can easily be
served in production.

The common solution to use is Bootstrap. Bootstrap is a powerful, extensible,
and feature-packed frontend toolkit. We can build and customize with Sass,
utilize prebuilt grid systems and components, and bring projects to life with
powerful JavaScript plugins.

## Creating the static directory

In the django_boards project root directory, alongside the templates directory,
we have to create a directory called static where our static files are kept.

## Creating the css directory

Inside the static directory, we create another directory called css. Now my
...
Next, we have to download [Bootstrap](https://getbootstrap.com). The current
version at the time of writing this post is 5.3.3, and the Compiled CSS and JS
version should be downloaded. From there, we should copy the
...
Next, we have to let Django know where the static files are located. In order to
do this, we have to go into settings.py and add the following as the values of
`STATIC_URL1` and `STATICFILES_DIRS`:
...
development for us is `http://127.0.0.1:8000`, when searching for static files.

The `STATICFILES_DIRS` tuple **[^1]** tells Django where to find static files
that are not linked to a particular app. Here we are telling Django to look for
static files in our root folder, not in any particular app (i.e., the board
app).
...
The collectstatic command is a built-in Django management command that collects
all static files from a Django project, including files from each installed app,
and places them in a single location, specified by the STATIC_ROOT setting in
settings.py. This is the location where our web server will look for static
files to serve to the end users in production.
...
template. Without this special tag, Django would not know to load the the static
...
**[^1]**: A Python tuple is a data structure similar to a list. The main
difference between the two is that a tuple is immutable, meaning it cannot be
changed once it is created. This makes it ideal for storing data that should not
be modified, such as database records (aka rows).
