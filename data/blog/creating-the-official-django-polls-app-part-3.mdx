...
date: '2025-01-06T12:26:17.011Z'
lastModified: '2025-01-08T10:38:17.011Z'
title: 'Creating the official Django Polls app Part 3'
author: 'Maria D. campbell'
excerpt: 'Creating the official Django Polls app Part 3.'
...
urlpatterns = [
    ...
    path('<int:question_id>/', views.detail, name='detail'), # new
    path('<int:question_id>/results/', views.results, name='results'), # new
    path('<int:question_id>/vote/', views.vote, name='vote') # new
]
```
...
![](Screenshot-2025-01-06-at-4.54.39-PM.jpg)
...
According to the tutorial, we are creating a templates directory in the polls
...
Next, I create a subdirectory inside templates called polls. Within polls, I
create a file called index.html. However, even though index.html resides in
polls/templates/polls/index.html, because of how the app_directories template
loader works, we can refer to this template within Django as polls/index.html.
...
The code above loads the template polls/index.html and passes it a context. The
...
render() takes the request object as its first argument, a template name as its
second argument, and a dictionary (context) as its optional third argument. It
returns an HttpResponse object of the given template rendered with the given
context.
...
1. template_name: The path to the template file we want to render (e.g.,
   'polls/index.html').
1. context (optional): A dictionary containing the data we want to pass to the
   template.
...
1. The render() function creates an HttpResponse object containing the rendered
   HTML content.
1. The HttpResponse object is returned, ready to be sent to the user's browser.

Benefits:

1. render() combines several common steps into one, making our view functions
   more concise.
...
Here, the detail view raises the Http404 exception if q question with the
...
There is a shortcut for using get() and raising an Http404 if an object does not
exist called get_object_or_404(). We can use it in the detail view:
...
get_object_or_404() takes a Django models as its first argument, followed by an
...
bridge our model and the database, allowing to perform queries and other
operations.
...
We can define custom managers gto add specialized behavior to our models. For
example:
...
As already mentioned, by default, Django adds a Manager with the name objects to
every Django model class. But if we want to use objects as a field name or to
use a name other than objects for the Manager, we can rename it on a per model
basis. To rename the Manager for a given class, we can define a class attribute
of type models.Manager() on that model. For example:
...
The templating engine uses dot lookup syntax ro access variable attributes.
...
here. If attribute lookup had also failed, it would’ve tried a list-index
lookup.

Examples of dictionary lookup, attribute lookup and list-index lookups using dot
notation:
...
`In` this `section`, we `write` our first `Django` app, `discuss` URL
...
template `tags` in the `template` engining `system`, `removing` the `hardcoded`
