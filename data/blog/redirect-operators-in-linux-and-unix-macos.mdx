...
lastModified: '2024-07-12T10:26:06.081Z'
...
In the case of a personal `macOS` device, **_by default_**, `stdin` refers to a
device's `keyboard`, and `stdout` refers to `Command Line` (`Terminal`). And
...
`home.txt` now `includes` a `second line` of `text`, which has been `appended`
