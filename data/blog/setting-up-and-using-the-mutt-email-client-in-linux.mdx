...
-   [Sending an email locally to magdala user which contains an attachment](#sending-an-email-locally-to-another-user-which-contains-an-attachment)
...
she wants to save to her local machine using the Up or Down arrow, and then she
hits the `V` key. This results in something like the following:
...
Next, user magdala hits the `S` key, and that is when the text "Save to file:
...
Next, user magdala hits the `Q` key, and is taken back to the Terminal command
prompt:
