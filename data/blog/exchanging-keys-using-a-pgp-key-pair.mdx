...
excerpt:
    'How to exchanging public keys using a PGP key pair via local email using
    mutt email client.'
...
_Photo by_
[Pavel Danilyuk Pavel Danilyuk](https://www.pexels.com/@pavel-danilyuk/) _on_
...
For options 1, 2, or 3, we have to export our public key using the `--export`
option. However, we need to transform it into a manageable format. We shouldn't
...
`ASCII armored output` for us when we use the `--armor` (or `-a`) option.
...
the binary `OpenPGP` format. By default, a key is exported in binary format. As
mentioned earlier, this is not transmittable via email.
...
revocation certificate in case if the key pair is lost or compromised.
