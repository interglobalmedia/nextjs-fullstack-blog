...
lastModified: '2026-05-27T17:54:27.998Z'
title: How to send/receive local emails in Linux Mint/VirtualBox
author: Maria D. Campbell
excerpt:
    How to send and receive local emails in Linux Mint/virtualBox using Command
    Line.
...
necessary. There is a simple built-in Linux Mint mail user agent program called
`mailx`. According to
[Geeks for Geeks](https://www.geeksforgeeks.org/mailx-in-linux/),

> The mailx utility is an enhanced version of the mail command. Along with the
> functionality provided by the original mail command, it provides extra
...
order to be able to use the `mail` command. What is interesting is that `mail`
and `mailx` are two separate commands, and they have their own `bin`. But
according to [Geeks for Geeks](https://www.geeksforgeeks.org/mailx-in-linux/),
...
In other words, they are interchangeable. When I did try to send an email to my
Gmail account, of course it did not work. I don't think I could accomplish that
these days from a local email address inside my Linux Mint/VirtualBox instance.
So that's why I decided to stick with this approach since I am working inside of
a virtual machine anyway. It will be interesting to find out if I can send an
email to Gmail in macOS when I set up cron jobs there. That's for another time!
...
And if I want to delete the message, I type the following after the `?` prompt:

```shell
? d 1
```

And then hit the `Enter key` followed by `Control key + D key`, which exits me
out of the shell. But before I actually exit out, the following is output:
...
It is also possible to send emails to other users on the system. For example, I
could send an email to user magdala. Then I could take advantage of things like
attaching a file to the email. Let's say I run the following in Terminal:
...
interpret the contents of my `history.txt` file properly, I use the `cat`
command which then becomes the `stdout` of the `mailx` command, thereby
redirecting the `stdin` `cat /home/maria/Desktop/history.txt` of `mailx` as
`stdout` of `mailx` to `magdala@maria-VirtualBox`.
...
- [mailx in Linux](https://www.geeksforgeeks.org/mailx-in-linux/): _Geeks for
  Geeks_
