...
became a real pain to have to leave Terminal to get the `gzipping` done. So I
decided to learn how to do it directly from the Command Line. The problem with
...
The command means that we are zipping recursively everything in the
`stripe-dropdown` directory, including subdirectories and files, but excluding
the `node_modules` directory and everything inside of it. We have to add the
full path to the `node_modules` directory in order to successfully ignore it.
`-r` stands for "recursively", and the `*` stands for "wildcard", meaning
...
`"stripe-dropddown/node_modules/*"`. On OSX, you have to encapsulate the path to
the directory to ignore with "quotes".
...
mine onto the desktop so as not to confuse it with my original and complete
...
name of the directory (stripe-dropdown) that had been zipped. No need for adding
the `.zip` extension. And that’s it! No more need to leave the Command Line and
go to your Mac's menu bar, searching for `compress stripe-dropdown` (or whatever
the name of the directory you want to compress is) under the file tab, and no
more need to leave Command Line to double click on a `.zip` to expand it into a
