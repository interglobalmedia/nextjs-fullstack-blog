...
lastModified: '2026-07-16T12:20:47.216Z'
...
image: oliver-sharp-0pUU4NIJuuw-unsplash.webp
tags: ['fullstack development', 'macOS', 'django', 'django-avatar', 'gravatar', 'humanize', 'python3', 'series']
isPublished: true
isFeatured: false
---

![Toy avatar in the grass](oliver-sharp-0pUU4NIJuuw-unsplash.webp)

_Photo by [Oliver Sharp](https://unsplash.com/@thechickennugget) on
[unsplash.com](https://unsplash.com/photos/a-toy-is-sitting-in-the-grass-with-a-toy-gun-0pUU4NIJuuw)_
...
- [Installing the django-avatar package](#installing-the-django-avatar-package)
    ...
    - [The default directory for django-avatar](#the-default-directory-for-django-avatar)
...
In this section, I am going to add the Humanize package to my Django project.
...
Since Humanize is a built-in package, the only way to upgrade to its latest
version is by upgrading to the latest version of Django:

```shell
python -m pip install -U Django
```

Next, I add `humanize` to `INSTALLED_APPS` in `django_boards/settings.py`:
...
Now I can use it in the templates.
...
<!--must be added AFTER base.html otherwise the package will not work -->
{% load humanize %}
...
<table class="table">
...
And that's it for `humanize`. You can add it wherever else you want. I am going
to just stick with it here for now.

## Adding a Gravatar

Now I am going to add [Gravatar](https://gravatar.com). `Gravatar` is an easy
...
Next, I am going to install the `django-avatar` package. It has the ability to
default to avatars provided by third party services (like Gravatar or Facebook)
if no avatar is found for a certain user. `django-avatar` automatically
generates thumbnails and stores them to your default file storage backend for
retrieval later.

To install `django-avatar`, I need to run the following command:
...
### The default directory for django-avatar

The default directory for `django-avatar` is "avatars", and the structure and
contents of that directory are the following:
...
There is much more to do regarding user `avatars/gravatars`. In the next
section, I will add a profile model, view, and template, and will add the
functionality for uploading a `gravatar/avatar` locally _as well as_ wrapping up
a few minor details relating to the `last_update` field for a post, further
pagination tweaks, and updating the `test_view_reply_topic_tests`.

## Code associated with this post

To view the code associated with this post, please visit
[be40ecc](https://github.com/interglobalmedia/django-boards/commit/be40ecc83cb7670c8959f9f5cd354d674ee0ad31).

## Conclusion

In this section, I add the `Humanize` package to my templates/topics.html, add
the Gravatar package for user gravatars, and add the django-avatar package for
automatic implementation of a user avatar when one is absent.
...
[^1]:
    Throughput is a measure of _how much_ work or information a system can
    process in a given amount of time. In programming, throughput can refer to
    the amount _of work_ completed in a given period of time, or the rate at
    which features are delivered.
