...
excerpt: How to create a cron job that creates backups of a directory in Linux.
...
my Linux Mint OS. The name of the backup includes the date and time of the
backup.
...
First, I created a backup directory called `desktop-backups` in my home
directory (`/home/maria`), and then I created a shell script called
...
[execve(2)](https://linux.die.net/man/2/execve) system call (which executes
programs).
...
and `seconds` in `24 hour format`. After I run the shell script, something like
the following appears in the `desktop-backup` directory:
...
First, I ran `crontab -e` in 1. It took me into the `crontab` file, where I
added my cron jobs. Then I added the following to the bottom of the file below
...
contents of the `desktop-backup` is deleted!
