...
Often, we might want to customize how the `admin form` looks and works. We’ll do
this by telling Django the options we want when we register the object.

We can learn about how this works by reordering the fields on the `edit form`.
We'll replace the `admin.site.register(Question)` line with:
...
We’ll follow this pattern to create a model admin class, then pass it as the
second argument to `admin.site.register()` any time we need to change the admin
options for a model.
...
We have our `Question` admin page, and a `Question` has multiple Choices. But
...
in the admin as a `<select>` box. In our case, only one question exists at this
point. In my case, two questions exist, one being in the future.

We also have an `“Add another question”` link next to `“Question.”` Every object
with a `ForeignKey relationship` to another gets this as well. When we click
`“Add another question”`, we get a popup window with an `“Add question”` form.
If we add a question in that window and click `“Save”`, Django will save the
...
But this is an inefficient way of adding Choice objects. It would be better if
we could add multiple `Choices` directly when we create the `Question` object.
We can make that happen.

First we remove the `register()` call for the `Choice` model. Then, we edit the
...
we come back to the “Change” page (`/change/`) for an already-created object, we
get another three extra slots.

At the end of the three current slots, we will find an `“Add another Choice”`
link. If we click on it, a new slot will be added. If we want to remove the
added slot, we can click on the "X" to the top right of the added slot. The
below shows an added slot:
...
tabular way of displaying inline related objects. To use it, we change the
...
Next, we'll tweak to the “change list” page. That's the one that displays all
...
be more helpful to display individual fields. To do that, we use the
...
We can click on the column headers to sort by those values, except in the case
of the `recently_published` header. That's because sorting by the output of an
...
We can improve upon `list_display` by using the `display()` decorator on that
...
Next, we edit our `polls/admin.py` file again and add an improvement to the
`Question` change list page. We add filters using the `list_filter`. We add the
...
The type of filter displayed depends on the type of field we’re filtering on.
...
search terms, Django will search the `question_text` field. We can use as many
fields as we’d like. But because it uses a LIKE query behind the scenes,
limiting the number of search fields to a reasonable number will make it easier
for our database to do the search.

Change lists also provide us with pagination. The default is to display 100
...
We can change it using Django’s template system. The Django admin is powered by
Django itself, and its interfaces use Django’s template system.

## Customizing our project’s templates

Next, we create a templates directory in our `django_polls` directory. Templates
can live anywhere on our filesystem that Django can access. Django runs as
whatever user our server runs. However, keeping our templates within the project
is a good convention to follow.

Next, we open our `settings.py` file (`django_polls/settings.py`) and add a DIRS
option in the TEMPLATES setting:
...
Next, we create a directory called admin inside templates, and copy the template
...
If we have difficulty finding where the Django source files are located on our
system, we can run the following command in Terminal:
