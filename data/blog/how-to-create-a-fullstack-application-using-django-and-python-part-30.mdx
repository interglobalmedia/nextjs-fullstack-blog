...
lastModified: '2026-05-25T03:50:16.435Z'
...
following URL pattern should be added in our `accounts/urls.py` file:
...
Adding these default `registration/password` reset URLs results in the inclusion
of the following URL patterns:
...
our templates directory should look like the following:
...
Since the `signup.html` template is not included in the list above, when a new
visitor lands on the site, they see the link to the sign in (login) page and not
the signup page. However, there is a link to the sign in page on the login page.
